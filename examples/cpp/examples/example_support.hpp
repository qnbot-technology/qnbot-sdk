...
constexpr const char* default_algorithm_id =
    "qnbot.hand.openxr_hand.retargeting";
...
struct SerialOptions {
    ...
    std::string algorithm_id{default_algorithm_id};
    bool algorithm_id_explicit{false};
...
inline SerialOptions parse_serial_options(int argc, char** argv,
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--port") {
            ...
            options.target_name = require_value(argc, argv, index, argument);
                   ...
                   example != SerialExample::haptics) {
            options.algorithm_id = require_value(argc, argv, index, argument);
            options.algorithm_id_explicit = true;
    ...
    if (options.external && options.algorithm_id_explicit) {
        throw std::invalid_argument(
            "--algorithm-id cannot be used with --external");
    }
    if (!options.external && options.port.empty()) {
...
inline qnbot::SdkConfig runtime_config(const SerialOptions& options) {
    ...
    if (!options.external) {
        target.algorithms = {qnbot::TargetAlgorithm{options.algorithm_id}};
    }
