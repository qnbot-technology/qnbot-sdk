...
inline qnbot::SdkConfig make_config(const std::string& package_id) {
    ...
    target.source = qnbot::DeviceSelector{
        "glove", std::string("external"), std::nullopt};
    target.name = "hand";
...
inline void advance_capture(
    const qnbot::ReadChannel<qnbot::CaptureProgress>& progress,
...
inline void check_calibration(
    ...
    const auto sample = progress.latest();
    if (!sample || sample->value.job.state != qnbot::CalibrationJobState::failed)
        return;
