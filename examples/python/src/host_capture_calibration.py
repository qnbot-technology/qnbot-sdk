...
def capture_side(
    ...
    print(f"{side.value}: capture readiness={readiness.state.value}")
    ...
    while time.monotonic() < deadline:
        ...
        if snapshot.state is CaptureSessionState.COMPLETED:
            ...
            print(f"{side.value}: capture completed")
            return session
        if snapshot.state in (
            ...
            raise RuntimeError(
                ...
                else f"{side.value} capture stopped"
            )
        ...
        if current is not None and current.state is CaptureStageState.AWAITING_CONFIRMATION:
            ...
            answer = input(f"{side.value} {prompt} [Y/n]: ").strip().lower()
    ...
    session.cancel()
    raise TimeoutError(f"{side.value} capture timed out")


def calibrate(
    glove: Glove, capture_session: CaptureSession, targets: tuple[str, ...]
) -> None:
    calibration = glove.start_calibration(
...
def main() -> None:
    ...
    if BUILTIN_SKELETON_PACKAGE_ID in package_ids:
        glove.device(LEFT_SOURCE).skeleton()
        glove.device(RIGHT_SOURCE).skeleton()

    left_session = capture_side(
        glove, package_ids, LEFT_SOURCE, Side.LEFT, options.force, start_runtime=True
    )
    right_session = capture_side(
        glove, package_ids, RIGHT_SOURCE, Side.RIGHT, options.force, start_runtime=False
    )
